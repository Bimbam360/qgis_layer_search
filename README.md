...
6. Click the color swatch button to pick a custom highlight color. Your choice is saved between sessions.
7. Check **Regex** to use regular expression matching, or **Fuzzy** to match approximate strings (typos, missing characters). The two modes are mutually exclusive.
...
- Highlight color overrides the default selection color and persists via QSettings.
- Fuzzy matching uses `difflib.SequenceMatcher` with a sliding window to handle partial queries against long layer names.
## Development
- [x] Basic Search
- [x] Regex Search
- [x] Custom highlight color
- [x] Fuzzy Search
- [ ] ??? Leave a request in issues!
## License
MIT License
## Author
Beau Seymour